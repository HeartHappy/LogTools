...
class FilterPageFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        allItems = arguments?.get("items") as? List<FilterChipItem> ?: emptyList()
        @Suppress("UNCHECKED_CAST")
        selectedValues = (arguments?.get("selectedValues") as? Set<String>) ?: emptySet()
        isEnabled = arguments?.getBoolean("isEnabled", true) ?: true

        setupAdapter()
        setupSearch()
        
        // 在初始化时就提交初始数据，确保显示正确的选中状态
